...
class Vaccination_reportController extends Controller
{
    public function index()
    {
        ...
        $reactionCount = $records->whereNotNull('side_effects')->where('side_effects', '!=', '')->count();
        $followUpDueCount = $records->whereNotNull('next_dose_date')
            ->where('next_dose_date', '>=', now())
            ->count();

        return view(
            'dashboard.vaccination_reports.index',
            compact('records', 'totalCount', 'completedCount', 'reactionCount', 'followUpDueCount')
        );
    }

    public function create()
    {
        $bookings = Booking::with('child')->latest()->get();
        $users = User::all();

        return view('dashboard.vaccination_reports.add', compact('bookings', 'users'));
    }
    ...
    public function store(Request $request)
    {
        ...
        VaccinationRecord::create($request->only([
            'booking_id',
            'administered_by',
            'vaccination_date',
            'dose_number',
    ...
    public function edit(VaccinationRecord $vaccinationRecord)
    {
        ...
        return view(
            ...
            compact('vaccinationRecord', 'bookings', 'users')
        );
    }
    ...
    public function update(Request $request, VaccinationRecord $vaccinationRecord)
    {
        ...
        $vaccinationRecord->update($request->only([
            'booking_id',
            'administered_by',
            'vaccination_date',
            'dose_number',
